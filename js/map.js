...
const control = document.getElementsByClassName('mapboxgl-ctrl-top-left');
    ...
    map.on('click', (e) => {
        ...
        if (map.getLayer('volcanoes').getLayoutProperty('visibility') !== 'none') {
            ...
            if (feature) {
                new mapboxgl.Popup()
                    .setLngLat(e.lngLat)
                    .setHTML(`<div class="linetab bold">Name: ${feature.properties.Name}</div>` + feature.properties.Html +
        ...
        if (feature) {
            ...
            const comments = feature.properties.comments.split('<br />').map((e) => {
                return textTolink(e);
            });
...
geocoder.on('result', (e) => {
    ...
    draw.add(feature);
    document.getElementById('disasterPlace').value = e.result.place_name;
    openleftBlock();
    getImages();
});
...
const setStyle = (basename) => {
    ...
    case 'MapboxMap':
        ...
        $('.bottom-right-control-basemap').addClass('display-none');
        return;
    default:
        if ($('.bottom-right-control-basemap').hasClass('display-none')) {
            $('.bottom-right-control-basemap').removeClass('display-none');
        }
...
const mapFlyToDisaster = (id) => {
    closePopup();
    const feature = map.getSource('disasterdb')._data.features.filter((e) => {
